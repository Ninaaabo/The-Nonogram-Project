...
app.post("/make-art", (req, res) =>{
  ...
  res.render("make-art.ejs", {x, y});
})

app.get("/sizing", (req, res) =>{
  res.render("sizing.ejs");
})
...
app.get("/all-art", async(req, res) =>{
  ...
  info = info.rows;
  res.render("gallery.ejs", {artList: info, url: `${req.protocol}://${req.get('host')}/get-nonogram?id=` })
})
...
app.get("/get-art", async(req, res)=>{
  ...
  for(var i = 0; i < info.y; i++){
    squares.push(new Array(x).fill(1));
  }

  for(var i = 0; i < info.x; i++){
    for(var j = 0; j < info.y; j++){
      console.log("square[", i, "][", j,'] = ', pic[i*info.y+j].isblack)
      squares[i][j] = pic[i*info.y+j].isblack;
    }
  }
...
app.get("/get-nonogram", async(req, res)=>{
  ...
  res.render("index.ejs", {x: numArr[0].length, y: numArr[1].length, numArr, id})
})
